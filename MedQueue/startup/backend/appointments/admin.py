...
class DoctorInviteCodeAdmin(admin.ModelAdmin):
    """Админка кодов приглашения для врачей"""
    list_display = ['code', 'specialty', 'hospital', 'is_used', 'used_by', 'created_at']
    list_filter  = ['is_used', 'specialty', 'hospital']
    search_fields = ['code', 'specialty']
    readonly_fields = ['created_at']
    list_editable   = []
    actions = ['generate_10_codes']

    def save_model(self, request, obj, form, change):
        if not obj.code:
            from .models import DoctorInviteCode as DIC
            obj.code = DIC.generate_code()
        super().save_model(request, obj, form, change)

    @admin.action(description='Сгенерировать 10 новых кодов')
    def generate_10_codes(self, request, queryset):
        from .models import DoctorInviteCode as DIC
        for _ in range(10):
            DIC.objects.create(code=DIC.generate_code())
        self.message_user(request, 'Создано 10 новых кодов врачей')
